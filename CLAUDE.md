...
- **`src/main/`** — all Node/OS access.
  ...
  - `workspace.js` — `WorkspaceManager`: spaces registry, `resolveSafe`, tree, text read/write, `ingest()`, chokidar watchers (workspace tree + `~/Downloads`); plus `originApp()`, the macOS quarantine-xattr provenance read the bridge filters on.
...
- **The Downloads bridge must never touch a file it can't attribute.** `~/Downloads` is the user's folder, shared with browsers, Slack, AirDrop and hand-made files, so the watcher's `add` event is *not* sufficient grounds to ingest. `applyBridgeSetting()` reads `originApp()` (macOS `com.apple.quarantine`, third `;`-field = downloading app) and ingests only when the name is in `bridgeAllowlist()` — `KNOWN_APPS` names + darwin bundle basenames + the user's `apps.json` entries, case-insensitive. No origin (a locally-created file) means skip. `ingest()` **copies**; nothing may disappear out of `~/Downloads`. The bridge is macOS-only — Linux has no provenance attribute and Windows' `Zone.Identifier` names a security zone rather than an app, so `applyBridgeSetting()` returns early there rather than guess (issue #1). Note web-tab downloads never reach this path at all; `setupProviderSession()` intercepts `will-download` and writes straight into `inbox/<provider>/`.
